<script>
  ...
  const getImageUrl = (product) => {
    if (product.documents?.length > 0 && product.documents[0]?.path) {
      const baseUrl = "https://storage.googleapis.com/static.production.regensburg.delcom.nl/";
  ...
  const getTitle = (product) => {
    if (product.translations?.length > 0) {
      return product.translations[0].description || "Keine Beschreibung verfügbar";
    }
...
{#if page === "home"}
  ...
  <a href="{base}/veranstaltung/{product.id}" class="productSlotLink">
    {#if imageUrl}
      <img src={imageUrl} alt={getTitle(product)} style="" loading="lazy" />
    {/if}
    <div class="productSlotHomeContent" style="position: relative; z-index: 10;">
...
{:else}
  ...
  <a href="{base}/veranstaltung/{product.id}" data-sveltekit-preload-data="tap" class="filterSlots">
    ...
    <div>
      ...
      <p>
        ...
        {product.location ? product.location.description : "No location available"}
      </p>
    </div>
    <!-- Bild -->
    {#if imageUrl}
      <img src={imageUrl} alt={getGermanDescription(product)} class="w-full h-48 rounded-lg object-cover" />
    {/if}
  </a>
{/if}
