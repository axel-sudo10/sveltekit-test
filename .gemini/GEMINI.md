Projekt informationen:
svelte api test projekt mit js und tailwind

erstelle keine +server.js!!!

⚠️ **WICHTIG: GIT NICHT VERWENDEN!**
- Benutze KEINE git Befehle (git add, git commit, etc.)
- Der User führt Git-Operationen selbst durch
- Fokus auf Code-Änderungen, nicht auf Git-Verwaltung

## Dokumentation - KONTEXTE-AI Ordner
Alle wichtigen Dokumentationen und Best Practices sind im `KONTEXTE-AI/` Ordner:

- **[ONBOARDING.md](../KONTEXTE-AI/ONBOARDING.md)** - Projekt-Übersicht, Setup, Komponenten, API
- **[ERROR_HANDLING.md](../KONTEXTE-AI/ERROR_HANDLING.md)** - Error Handling Patterns
- **[FILTER-BEST-PRACTICES.md](../KONTEXTE-AI/FILTER-BEST-PRACTICES.md)** - Wie Filter erstellt werden

**Lese diese Dateien bevor du etwas in diesen Bereichen machst!**

## Workflow
1. Denke über Aufgabe/Problem nach
2. Nutze SvelteKit MCP für Dokumentation
3. Bei Bedarf zusätzlich Context7 MCP für weitere Docs
4. Erstelle Todo-Liste und arbeite sie ab

## Dokumentation-Herangehen
- Nur das Nötigste schreiben
- Knapp und strukturiert
- Keine Überdokumentation
- Fokus auf Kern-Infos

## CSS Klassen Organisation
Siehe: [KONTEXTE-AI/ONBOARDING.md](../KONTEXTE-AI/ONBOARDING.md) - Abschnitt "CSS Klassen & Organisation"

Alle Custom CSS Klassen in `src/app.css` sind in 3 Kategorien organisiert:
1. **LAYOUT CONTAINER** - Flex/Grid Layouts
2. **FILTER KOMPONENTEN** - Filter-spezifische Elemente
3. **TEXT & TYPOGRAFIE** - Text-Styling

Neue Klassen immer in passende Kategorie einsortieren + Kommentar hinzufügen.
Siehe `src/app.css` Header für Details.
